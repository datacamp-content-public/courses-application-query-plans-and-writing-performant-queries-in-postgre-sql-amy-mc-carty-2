...
`@script`
Using range as the PARTITION METHOD shown in the parent table creation on the prior slide, two children tables are created. The start and end values of the order_date are specified. 
...
The second examples shows a syntax example for a child table if the PARTITION METHOD is list instead of range.
...
`@script`
Once the tables are created, indexes should be created on the PARTITION KEY and must must be created on each child table, as shown in the example here where an index is set for the 2017 and 2018 children tables individually.

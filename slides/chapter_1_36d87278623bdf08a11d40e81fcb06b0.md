...
---
## Setting up a Partition: Partitioned Table & **Partitions**
...
`@script`
With the range PARTITION METHOD, as shown in the table creation on the prior slide, two children tables are created. The start and end values of the order_date are specified. 
...
`@script`
Once the tables are created, indexes should be created on the PARTITION KEY and must be created on each child table, as shown in the example here where an index is set for the 2017 and 2018 children tables individually.
...
The inheritance approach can still be used, and it may be chosen because it does retain a few advantages.

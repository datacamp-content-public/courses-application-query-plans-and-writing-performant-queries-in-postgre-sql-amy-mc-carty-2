...
---
## PostgreSQL Partitioning Terminology and Methods
...
The **partitioned **Table is divided table and it is divided into individual pieces, the partitions.
Partition Key – the column(s) or expressions used to create the divisions / how new records are routed
Partition Bounds – the parameters defining, or constraining, each partition
Partition Method – the techniques that use the key to create the divisions
	range – start / end values (i.e. dates)
	list – explicit all inclusive list (i.e. geography)
